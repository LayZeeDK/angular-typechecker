...
const warningCount = diagnostics.filter((diagnostic) => diagnostic.category === ts.DiagnosticCategory.Warning).length; // NOT length - errorCount; Suggestion/Message stay uncounted (invariant: errorCount + warningCount <= diagnostics.length)
```
