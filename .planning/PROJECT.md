...
### Active (v0.1.2 -- Storybook story type-checking)

- [ ] Input-set-membership boundary filter (pure `keep()`, walk surfaces rootName paths, walk + single-leaf unified, zero ngtsc internals). -- SB-02
- [ ] Split + surfaced suppression counts; `suppressedInGraph > 0` -> non-clean coverage-incomplete (charter floor). -- SB-04
- [ ] Type-check `*.stories.ts` under the per-project default scaffold (Layout A), app + lib, zero consumer edits. -- SB-01
- [ ] Type-check aggregated cross-project stories/components of a centralized Storybook host (Layout B, Nx `one-storybook-for-all`), incl. external-template diagnostics. -- SB-03
- [ ] Phase-16 gate spike (G1-G5, GO/NO-GO) on the official stack with forced `@storybook/angular@10.4.6`. -- SB-05
- [ ] Negative-fixture validation per layout + extended packaged-tarball e2e. -- SB-06
- [ ] README/changelog coverage claim + caveats + green->red callout. -- SB-07

Detail: `.planning/REQUIREMENTS.md`. Board rationale: `.planning/research/v0.1.2-storybook/board/CONSENSUS.md`.
...
- Storybook `*.stories.ts` type-check support -- PROMOTED to the active v0.1.2 milestone (see Current Milestone above); no longer out of scope.
...
| v0.1.2: Storybook story type-check is ONE input-set-membership boundary fix (not Storybook-specific code); no version gate, no `*.stories.ts` selector, no new public option | 6-lens Opus board consensus: directory-containment is a lossy proxy for "files this run was asked to check"; the compiler's declared inputs (rootNames) are authoritative and Storybook-agnostic | Scoped v0.1.2 (board `research/v0.1.2-storybook/board/CONSENSUS.md`) |
| v0.1.2: support BOTH the per-project scaffold (Layout A, free) and the centralized-host recipe (Layout B) as minimums; Layout C excluded (guard only) | Layout B is the official Nx recipe AND a live silent false pass today (filter keys off the host dir; recipe aggregates stories from OTHER dirs) -- fixing it is charter- and ecosystem-mandatory | Scoped v0.1.2 (spike-gated) |
| v0.1.2: enforce never-false-pass via a SPLIT suppression counter -- `suppressedThirdParty` (node_modules, quiet) vs `suppressedInGraph` (first-party dropped); `suppressedInGraph>0` -> non-clean coverage-incomplete | A `logger.warn` beside `success:true` is functionally silent to the only consumers (CI exit code, agents' structured verdict); the guard must be verdict-affecting | Scoped v0.1.2 |
| v0.1.2: external-template coverage is G1-gated; the owning-component->template map may use ONLY a stable public signal, else a keep-all-external-template fallback | The vendored `Program` interface deliberately exposes no template->component registry; reaching ngtsc internals would silently break across Angular patches -> false pass | Scoped v0.1.2 (G5 selects; both shippable) |
| v0.1.2: Phase-16 spike is a hard GO/NO-GO gate on the official stack with forced `@storybook/angular@10.4.6` | The whole Layout-B premise rests on unverified empirics (G2 rootNames materialization, G3 forced-SB10 compiles, G4 NG8xxx fire); a NO ships Layout A + documents Layout B "not yet" | Scoped v0.1.2 |
...
---
*Last updated: 2026-07-05 -- v0.1.2 MILESTONE OPENED: Storybook story type-checking. Scope + decisions hardened by a 6-lens Opus advisory board (3 constructive + 3 adversarial lenses, 2 rounds, consensus). Reframed as ONE input-set-membership boundary correctness fix (not Storybook-specific code); Layout A (per-project scaffold) + Layout B (centralized host, Nx one-storybook-for-all recipe) both minimum, Layout C excluded (guard only); never-false-pass enforced via a split suppression counter (`suppressedInGraph>0` -> coverage-incomplete); external-template coverage G1-gated with a no-ngtsc-internals rule; Phase-16 spike (G1-G5) is a hard GO/NO-GO gate on the official stack with forced @storybook/angular@10.4.6. Requirements SB-01..08 (`.planning/REQUIREMENTS.md`), phases 16-19 (`.planning/ROADMAP.md`), full board record (`.planning/research/v0.1.2-storybook/board/`). STATE.md reset to v0.1.2. Next: `/gsd-plan-phase 16`.*
