...
- A value import of the type it reads: `import type { CoreResult } from './run-typecheck';`
  (`evaluate-result.ts:23`). For `exit-codes.ts` ALSO add a _value_ import of
...
describe('evaluateResult', () => {
  it('fails when errorCount > 0 ... (EXE-05 / D-03: errors always fail)', () => {
    expect(evaluateResult({ errorCount: 1, warningCount: 0 }).success).toBe(false);
  });
...
const malformedTsConfig = join(workspaceRoot, 'fixtures', 'config-broken', 'tsconfig.malformed.json');
```
...
with `errorCount >= 1`. RESEARCH Pitfall 1: a _nonexistent extends target_ is 5012, a
_nonexistent tsconfig PATH_ (ENOENT) is the 500 -- they are different cases and both must
...
```jsonc
{
  "compilerOptions": { "noEmit": true, "target": "es2022", "module": "preserve", "moduleResolution": "bundler", "strict": true },
...
it('catches a TypecheckInfrastructureError -> logger.error + { success: false } (D-01)', async () => {
  ...
  mocks.runTypecheck.mockRejectedValue(new TypecheckInfrastructureError('simulated internal crash'));
...
```typescript
expect(mocks.loggerError).toHaveBeenCalledWith(expect.stringContaining('infrastructure error'));
```
