...
- PITFALL (order-dependent): for a survivor that WAS iterated BEFORE the poison in the aborted
  ...
  set DIRECTLY on the impl's fileData (`checker.ts:946`) -- but `updateFromContext(ctx)` (line
  949) never ran, so its shims were generated into the now-DISCARDED `ctx` and never committed to
