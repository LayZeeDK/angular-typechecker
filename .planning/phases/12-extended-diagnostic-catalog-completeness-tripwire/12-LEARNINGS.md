...
A `BaselineRow { label, code, isNg, fixtureScenario, expectWarning? }` `describe.each` reusing the extended spec's resolver and `NG` import; WARN\_-prefixed codes (NG6100) add a Warning-category + warningCount assertion.
