...
- Complete Storybook coverage for both setups. Whether your stories live in each
  ...
  files your Storybook `tsconfig` declares. An error in a pulled-in story or
  component is reported with the correct file and code frame, including stories that
  use an external `templateUrl` template.
- A notice for files that can't be type-checked. When your Storybook `tsconfig`
  ...
  naming them, so a gap is never silent. This covers `.mdx` docs (never
...
- A referenced project config that resolves to no input files (an empty config, or a
  ...
  instead of being skipped quietly, so an accidentally-empty target can't report a
  false pass. Out-of-project, duplicate, and self references are still skipped
  quietly, as before.
...
- The plugin has no dependency on Storybook. Installing `@storybook/angular` itself
  on Angular 22 currently needs `--legacy-peer-deps` (or `--force`), because
  Storybook hasn't yet widened its Angular peer range. See the README "Storybook"
  section.
