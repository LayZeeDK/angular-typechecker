...
**Resolved:** 2026-07-18 (branch `gsd/v0.2.3-machine-readable-reporters`)

- **MJ-01 -- FIXED.** `fingerprintOf` now appends the 1-based `record.column`
  to the `atcFingerprint/v1` sha256 tuple (`sarif-report.ts`), so two distinct
  diagnostics sharing rule + message on the same line get distinct fingerprints.
  The key stays `atcFingerprint/v1` -- the recipe is still unreleased, so v1 was
  corrected in place (no `/v2`). The golden snapshot was regenerated by RUNNING
  the tests; both fingerprints were independently recomputed byte-for-byte, and
  the tuple carries no absolute path / cwd / volatile field, so determinism holds.
  D-02 in `31-CONTEXT.md` and the recipe doc-comment were updated to list the
  1-based column.
  New fingerprints: TS2322 (positioned) ->
  `b48cd85e66c220994c36c958ec69ce1e33d5b80a74036e7f4fb5f1a9b8f0857d`;
  ATC90001 (file-less) ->
  `02b8de2056417eaeea4135cd878d59ed98905723e62912535c75464f45ea6045`.
- **MN-01 -- FIXED.** `extended-catalog.spec.ts` now asserts the 18 `ngCode`
  values are pairwise distinct (`new Set(ngCodes).size === ngCodes.length`).
- **NT-01, NT-02 -- INTENTIONALLY NOT APPLIED (YAGNI).** The reviewer verified
  both are currently unreachable/safe; defensive code for verified-safe edge
  cases is unwarranted. Re-open only if the cited contracts change
  (`relativizePath` returning `''`, or a diagnostic source setting `file`
  without `start`).

Gates: `nx test` (522 pass), `nx typecheck`, `nx lint` (maxWarnings:0),
`nx format:check` -- all green. Additive-only charter (D-08) preserved: no
public barrel / `index.drift.ts` / `builder.ts` / schema-parity / version change.
