...
- [x] **CAT-02**: At least one severity-promotion case proves `angularCompilerOptions.extendedDiagnostics.defaultCategory: "error"` flips a warning-default diagnostic to an error (satisfied by the NG8101 proof). *(Reconciled at the v0.1.0 milestone audit, 2026-07-02, per Phase 12 D-13: the earlier "NG8011 excepted: out-of-band / not promotable" parenthetical was factually WRONG -- NG8011 IS promotable, as are all 18 `ExtendedTemplateDiagnosticName` members via `defaultCategory`; the implementation always treated NG8011 as a normal promotable Warning-default row. CAT-02 remains satisfied by the single NG8101 promotion proof.)*
