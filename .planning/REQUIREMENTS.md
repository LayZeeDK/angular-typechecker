...
- [ ] **COR-04**: The executor signals an infrastructure failure (`TypecheckInfrastructureError`) with an exit code DISTINCT from a type-error failure (ngc-style: infra vs. type error), so CI/agents can tell a crash apart from real type errors. -- [Q3 decision; parallels ngc `exitCodeFromResult` 2-vs-1].
