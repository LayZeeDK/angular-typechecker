...
import { buildCleanEnv, sh } from './e2e-process';
import { findWorkspaceRoot } from './find-workspace-root';

// The SINGLE source of truth for the Verdaccio vitest globalSetup shared by every
// registry-publishing e2e project (angular-typechecker-install-e2e +
// angular-typechecker-ng-cli-e2e). It stands up the first-party @nx/js Verdaccio
// local-registry, mints a REAL publish token, strips CI-only provenance, and
// publishes the dist (built ONCE upstream by the `e2e` target's dependsOn) via the
// real `nx release publish` path -- then provides the registry URL + token to every
// spec (finding E1: one build + one publish shared across the specs, which is why the
// sibling specs drop their own per-spec builds).
//
// The two e2e projects were byte-identical in code (project name is DERIVED from the
// root package.json, not hardcoded); they differed only in a header comment. This
// factory removes that verbatim duplication. `createVerdaccioGlobalSetup` returns the
// vitest globalSetup default function; each e2e global-setup delegates to it in one
// line.
//
...
export interface VerdaccioGlobalSetupOptions {
  // Optional project label surfaced ONLY in setup-failure error context (a
  // diagnostic aid identifying which e2e project's setup failed). The load-bearing
  // publish-once flow + safety gate are byte-identical whether or not it is passed.
  label?: string;
}
...
export function createVerdaccioGlobalSetup(
  options: VerdaccioGlobalSetupOptions = {},
) {
  const setupLabel = options.label ? ` (${options.label})` : '';
  ...
  return async function ({ provide }: TestProject) {
    ...
    try {
      ...
      if (!registryUrl.startsWith('http://127.0.0.1:')) {
        throw new Error(
          `refusing to publish to non-local registry: ${registryUrl}${setupLabel}`,
        );
      }
