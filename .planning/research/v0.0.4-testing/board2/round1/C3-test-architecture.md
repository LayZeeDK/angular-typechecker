...
2. **In-plugin specs are CI-free to add** - a new `*.spec.ts` / `*.integration.spec.ts` under
   ...
   *e2e project* is invisible to CI until added by name to the `e2e` job's explicit `-p` list. (FACTS sec.5;
...
- (b) carries a permanent maintenance tax that buys nothing here: it is an *internal* deep import
...
- Determinism/parallelism: the in-memory tree is fully isolated per `createTree*` call and does real *no*
...
- Prior art does NOT contradict this: the sandbox's committed generator spec uses in-memory
  `createTreeWithEmptyWorkspace`, and its `FsTree`/`flushChanges` proposal was *never committed*
  (SANDBOX sec.2, sec.9 item 5). Connect used in-memory for the generator and reserved `FsTree` for the
  *executor* e2e where a real `ngc` must observe edits (CONNECT 2c). Both prior arts already place the
...
- Whether any generator test needs to assert behavior of a *real* type-check over the *generated* config
...
that a subsequent step *inside the same generator/test boundary* reads back (e.g. it generates a tsconfig
...
they are complementary, not exclusive. It extends the convention the repo *already started* (v13 files
...
- The repo *deliberately renamed* `extended.angular17` -> `extended.promotion` because the version signal
...
- The exact `ErrorCode` enum -> NG-number mapping for all 18 `ExtendedTemplateDiagnosticName` members in
  the *installed* `@angular/compiler-cli@22.0.4` (FACTS sec.4 says read during work; the catalog labels
...
**Fact that would change my position:** If a material fraction of the 18 codes turn out to be *infeasible*
...
- Adding a NEW e2e project costs a `ci.yml` edit (the `e2e` job's explicit `-p` list; FACTS sec.5,
  ...
  Extending the *existing* `install-e2e` project adds a spec file with ZERO ci.yml change and reuses the
...
- The generator is the milestone's named deliverable (FACTS sec.1) and is a small, well-understood, low-risk
  artifact - the sandbox's is 33 lines (FACTS sec.7a; SANDBOX sec.1). It is the *consumer* that justifies
...
- Things to EXCLUDE are explicitly flagged by prior art as un-covered/expensive with no prior art to lean
  ...
  most expensive, lowest-prior-art surfaces. (Note: per-project-type *coverage* is already partly bought
