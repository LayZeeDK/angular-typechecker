...
> **NOTE (D-13) -- RECONCILED at the v0.1.0 milestone audit (2026-07-02):** SC3's parenthetical "(out-of-band / not promotable)" above is FACTUALLY WRONG and is hereby superseded -- NG8011 IS promotable (triple-verified docs+source+runtime; CONTEXT.md D-09 corrected; all 18 `ExtendedTemplateDiagnosticName` members are promotable via `defaultCategory`). The implementation always treated NG8011 as a normal promotable Warning-default member. The matching parenthetical in REQUIREMENTS.md CAT-02 has been corrected as part of this audit. CAT-02 stays satisfied by the single NG8101 promotion proof (D-08). No re-execution needed -- the code was already correct; only the historical SC/requirement prose carried the error.
...
- [x] 14-02-PLAN.md -- `configuration` generator: init-first, resolve tsConfig (override -> solution `tsconfig.json` -> flat leaf fallback -> error), collision-by-executor, write ONE `typecheck` target via `updateProjectConfiguration` + schema/parity + in-memory specs (GEN-01/02/03/04/08, GEN-05/06 slice)
- [x] 14-03-PLAN.md -- Registration + packaging: root `generators.json` (factory-keyed), `package.json` `generators` field + `files` entry, `project.json` build asset glob, `package-manifest.spec.ts` extension; `nx lint`/`nx build` gates (GEN-05 registration, GEN-09 nx-add mechanism)
...
**Plans**: 2 plans
- [x] 15-01-PLAN.md -- GUARD-01: in-plugin bidirectional `-p` set-equality guard (6-cell matrix) + mandatory deliberate-RED proof (GUARD-01)
- [x] 15-02-PLAN.md -- `consumer-generator` fixture + `generator-e2e` (GE2E-01/02) + `nx-add-e2e` (GE2E-03) heavy tarball specs + D-13 tarball-audit hardening (GE2E-01, GE2E-02, GE2E-03)
