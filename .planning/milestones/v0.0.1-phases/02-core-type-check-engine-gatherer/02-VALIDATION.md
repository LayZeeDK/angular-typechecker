...
- [x] `src/core/*.integration.spec.ts` per-version split (`baseline.angular13`, `extended.angular13`) plus the version-independent `extended.promotion` proof with the `NG()` helper (`diagnostic-codes.ts`) calling `runTypecheck` directly (D-07c)
...
- `extended.promotion.integration.spec.ts` (renamed from `extended.angular17.integration.spec.ts`) proves the version-independent `defaultCategory:"error"` promotion using the portable NG8101 shape. The old `angular17` filename was a false coverage signal (no v17-specific code, no `fixtures/extended-v17/` tree); the rename drops it. Genuine per-version extended-code coverage remains separate, future feature work (IN-01 fixed).
