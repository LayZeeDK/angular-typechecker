...
- `typecheck-configuration` Nx generator (Nx 23 devkit): edits `project.json` ONLY (`readProjectConfiguration`/`updateProjectConfiguration`/`formatFiles`; NO `generateFiles`, no file emission); hand-authored schema; idempotent re-runs. **Open generator-phase design decision:** per-project-type `tsConfig` defaulting (application -> `tsconfig.app.json`, library -> `tsconfig.lib.json`) + spec-tsconfig (`tsconfig.spec.json`) handling (single target + `--tsConfig` option vs. multiple targets vs. `configurations`); prod tsconfigs (`tsconfig.lib.prod.json`) skipped (no-emit); **Nx workspaces only -- Angular CLI `angular.json` layouts deferred**.
- Generator tests on the PUBLIC in-memory `createTreeWithEmptyWorkspace` substrate (+ schema-parity). The bespoke real-disk `createFsTree`/`flushFsTreeChanges` is NOT built (board Option A: zero value for a `project.json`-edit generator; the prior-art FsTree helper lived only in an executor e2e). Real-disk fidelity comes from the tarball e2e.
