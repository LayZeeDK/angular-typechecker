...
**Notes:** IMPACT high, CONFIDENCE high (the decision is the *process*, not the shape). Grounded in PRIOR-ART #3 + ENGINE-REF Open Q3 (the `d.file === file` filter at `compiler.ts:618`). HYBRID is the safe superset because the whole-program non-template call is what the engine already does.
