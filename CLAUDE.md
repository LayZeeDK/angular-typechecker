...
- `@nx/devkit@23.0.1` declares `nx` as a **peerDependency**: `">= 22 <= 24 || ^23.0.0-0"`. So when your plugin depends on `@nx/devkit`, the consumer's `nx` is satisfied transitively through that peer -- you never declare `nx` yourself. `[v0.2.1 CORRECTION (2026-07-12): nx is now a direct ^23.0.0 dependency -- yarn does not auto-install the @nx/devkit peer; see PROJECT.md Constraints]`
...
| ----------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@nx/devkit`            | `23.0.1` (pin exact)                         | Plugin authoring API (`ExecutorContext`, `logger`, `readJsonFile`, etc.)                  | `[adds]` Ship as a pinned `dependency`. Its own `nx` peer (`>= 22 <= 24                                                                                                                                                                                                |     | ^23.0.0-0`) is what satisfies the consumer's Nx -- you never declare `nx`. `[v0.2.1 CORRECTION (2026-07-12): nx is now a direct ^23.0.0 dependency -- yarn does not auto-install the @nx/devkit peer; see PROJECT.md Constraints]` |
...
| `nx`                    | `23.0.1` (latest)                            | Workspace runtime (your dev repo + consumer)                                              | `[confirms PROJECT.md "Nx 23.x"]` Declared by no one in your package.json -- flows in via `@nx/devkit`'s peer. `[v0.2.1 CORRECTION (2026-07-12): nx is now a direct ^23.0.0 dependency -- yarn does not auto-install the @nx/devkit peer; see PROJECT.md Constraints]` |
...
# dependencies: @nx/devkit@23.0.1, nx@^23.0.0, tslib@^2.3.0

# peerDependencies: @angular/compiler-cli@^22.0.0, typescript@>=6.0.0 <6.1.0

# (nx IS a direct ^23.0.0 dependency since v0.2.1 -- yarn does not auto-install the @nx/devkit peer; still NO @nx/plugin, NO vitest, NO nx in peerDependencies)
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nx` in `dependencies` OR `peerDependencies` of the published plugin `[v0.2.1 CORRECTION (2026-07-12): nx IS now a direct ^23.0.0 dependency -- yarn does not auto-install the @nx/devkit peer; peerDependencies still forbidden; see PROJECT.md Constraints]` | Official recipe forbids it; `@nx/devkit`'s own peer (`>= 22 <= 24                                                      |                                                                                                             | ^23.0.0-0`) already constrains the consumer's `nx`. Declaring it yourself causes double-constraint/skew. | Depend on `@nx/devkit` (pinned) AND declare `nx` directly at `^23.0.0` (v0.2.1: yarn does not auto-install the @nx/devkit peer); keep `nx` OUT of peerDependencies. |
...
| ------------------------------ | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | --- | --- | --- | --- | --------------------------- |
| `nx`                           | `23.0.1` (workspace runtime; latest)  | `[confirms PROJECT.md "Nx 23.x"]` Not declared in the plugin's package.json. `[v0.2.1 CORRECTION (2026-07-12): nx IS now a direct ^23.0.0 dependency -- yarn does not auto-install the @nx/devkit peer; see PROJECT.md Constraints]` |
