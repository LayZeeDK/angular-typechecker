...
Last activity: 2026-07-15 -- Quick task 260715-mia: surface-symmetry cleanup -- dropped the redundant Angular CLI `init` schematic (init is Nx-only now) and relocated the `ng-add` schema out of the Nx `src/generators/` tree into `src/schematics/ng-add/` (rename `NgAddGeneratorSchema` -> `NgAddSchema`; ng-add is Angular-CLI-only now). Net: collection.json = {ng-add, configuration}, generators.json = {init, configuration}; Nx surface + init generator byte-unchanged. build/test(373)/lint/format + dist-ship green. No product/version change (0.2.0)
