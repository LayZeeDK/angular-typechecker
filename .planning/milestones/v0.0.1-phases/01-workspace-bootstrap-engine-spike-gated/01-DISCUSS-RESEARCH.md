...
**`@nx/js:tsc` wrinkle: NONE (verified).** `tsc.impl.ts` only READS `tsConfig.options.module` (to label packaging); it never reassigns it for compilation. `module: node16` in `tsconfig.lib.json` reaches emit untouched. (The only `module =` write in `@nx/js` is in the library _generator_, not the build executor.)
