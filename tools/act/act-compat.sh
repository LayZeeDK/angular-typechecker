...
assert_selected "$PR_PLAN" "ci/cve-lite" "pull_request"
...
# The RED PROOF job (Phase 35) is PR-only: its if: (github.event_name == 'pull_request'
...
assert_selected "$PR_PLAN" "ci/code-scanning-red-proof" "pull_request"
...
# The RED PROOF job's github.event_name == 'pull_request' if: gate is FALSE on a push
...
assert_absent "$PUSH_MAIN_PLAN" "ci/code-scanning-red-proof" "push-main"
