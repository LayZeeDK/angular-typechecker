...
- **`act` runs native arm64 on Windows arm64** — the `catthehacker/ubuntu:act-*` images are multi-arch (`linux/arm64/v8`); Docker auto-selects arm64. The "amd64-only -> QEMU" claim was stale folklore; the trap is *forcing* `--container-architecture linux/amd64`.
...
- **Windows GitHub runners are NTFS** (not ReFS/Dev Drive). NTFS ≈ ReFS for case-insensitivity + realpath + symlink resolution; the only gap (8.3 short names, NTFS-only) is unreachable by TS/ngtsc/Node. CI-on-NTFS is a *superset* of a local ReFS Dev Drive — no FS-divergence blind spot.
