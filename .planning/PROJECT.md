...
- **Current milestone:** **v0.1.0 -- configuration + init generators, nx add support, and the typecheck executor rename** (FEATURE-COMPLETE -- Phases 12, 13, 13.1, 14, 15 all shipped; only the 0.1.0 Release PR/cut remains). The executor rename `angular-typechecker:angular-typecheck` -> `angular-typechecker:typecheck` (EXEC-01, breaking -> 0.1.0) is COMPLETE (Phase 13.1). Phase 14 is COMPLETE: the `configuration` generator (wires ONE minimal `typecheck` target at the solution `tsconfig.json` into `project.json`) + the standalone `init` generator (seeds `nx.json` `targetDefaults` with the verbatim WALK-02 cacheable block, unscoped id, whole-entry `??=` don't-clobber) + `nx add angular-typechecker` (runs the registered `init` by literal key -- no `ng-add` alias) all landed, config-edit only, verified 5/5 SC + 236 in-plugin tests green (GEN-01..09). Phase 15 is COMPLETE: the folded generator + `nx add` e2e against the installed tarball (GE2E-01/02/03) and the CI `-p` set-equality self-audit guard (GUARD-01) all landed, verified 4/4 SC (install-e2e 26 tests green; plugin suite 239 green incl. the guard + its deliberate-RED proof). All 22 v0.1.0 requirements are now validated. Builds on the shipped reference-walking engine (Phase 13) and the complete NG8xxx catalog + tripwire (Phase 12). See `## Current Milestone` below.
