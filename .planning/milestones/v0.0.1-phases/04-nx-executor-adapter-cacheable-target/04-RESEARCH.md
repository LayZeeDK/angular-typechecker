...
export function normalizeOptions(
  ...
  const tsConfigPath = isAbsolute(options.tsConfig)
    ? options.tsConfig
    : joinPathFragments(context.root, options.tsConfig); // D-03
...
function run(extra = ''): { stdout: string; code: number } {
  try {
    const stdout = execSync(
      `npx nx run ${TARGET} --output-style=static --no-color ${extra}`,
      { env, encoding: 'utf8' },
    );
...
describe('TEST-04: a dep type error busts the consumer cache', () => {
  it('R1 pre-flight: the dep source IS an input for the consumer target (edge exists)', () => {
    ...
    const out = execSync(
      `npx nx show target inputs ${TARGET} --check ${DEP_FILE}`,
      { env, encoding: 'utf8' },
    );
