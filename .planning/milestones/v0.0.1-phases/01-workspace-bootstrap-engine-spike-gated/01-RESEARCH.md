...
const BUILT = join(__dirname, '../../../../dist/packages/angular-typechecker/src/executors/angular-typecheck/executor.js');
...
<p>{{ status }}</p>
<!-- NG8109 INTERPOLATED_SIGNAL_NOT_INVOKED (must be status(), not status) -->
```
...
_If the generator already emits a runnable `vitest.config.ts` + sample spec (it does for `--unitTestRunner=vitest`), the only NEW files are the two gate specs + the fixture._
