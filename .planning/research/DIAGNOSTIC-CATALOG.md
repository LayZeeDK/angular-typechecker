...
| NG8116 | missingStructuralDirective | v19 (19.2.0; appeared in docs only at v20) |
...
Authoritative Angular 22 set, verified against https://angular.dev/extended-diagnostics (2026-06-27). Gaps: **NG8110 and NG8112 are NOT on the extended-diagnostics docs list** (their code numbers are held by undocumented compiler diagnostics -- NG8110 = `UNSUPPORTED_INITIALIZER_API_USAGE`, NG8112 = `UNUSED_LET_DECLARATION`); v15 added none; there are **no extra v22-only extended diagnostics** beyond NG8021 (the v22 documented set == v21 set, 16 entries). Each documented diagnostic has a page at `/extended-diagnostics/<code>`.

**Introduction-version provenance + method:** the "Introduced" column is derived by diffing the **versioned** extended-diagnostics docs across majors -- `https://v<NN>.angular.dev/extended-diagnostics` for NN = 13..21 (URL format differs on older majors; the unversioned `angular.dev/extended-diagnostics` is the current = v22 set). A code's introduction major is the first versioned page it appears on. **URL domains differ by era:** older majors are on `angular.io` (e.g. v13 = `https://v13.angular.io/extended-diagnostics`), newer majors on `angular.dev` (the docs domain moved around v17/v18: `https://v<NN>.angular.dev/extended-diagnostics`); the unversioned `https://angular.dev/extended-diagnostics` = current (v22). **Verified 2026-06-27** against the versioned docs AND the Angular compiler `git tag --contains` (local clone). The git-tag signal is authoritative because the docs page can LAG the compiler release: NG8103-NG8107 shipped in **14.1.0** though the v14 overview page only listed NG8101/8102 (do NOT re-date them to v15), and NG8116 shipped in **19.2.0** but appeared in the docs only at v20. To extend for a future major, prefer `git tag --contains <commit>` on the compiler source over the docs-diff alone.
