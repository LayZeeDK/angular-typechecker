...
- **Current milestone:** **v0.1.0 -- configuration + init generators, nx add support, and the typecheck executor rename** (in progress -- Phases 12, 13, 13.1, 14 shipped; only 15 remains). The executor rename `angular-typechecker:angular-typecheck` -> `angular-typechecker:typecheck` (EXEC-01, breaking -> 0.1.0) is COMPLETE (Phase 13.1). Phase 14 is COMPLETE: the `configuration` generator (wires ONE minimal `typecheck` target at the solution `tsconfig.json` into `project.json`) + the standalone `init` generator (seeds `nx.json` `targetDefaults` with the verbatim WALK-02 cacheable block, unscoped id, whole-entry `??=` don't-clobber) + `nx add angular-typechecker` (runs the registered `init` by literal key -- no `ng-add` alias) all landed, config-edit only, verified 5/5 SC + 236 in-plugin tests green (GEN-01..09). Still to ship (Phase 15): the folded generator + `nx add` e2e against the installed tarball and the CI `-p` set-equality guard. Builds on the shipped reference-walking engine (Phase 13) and the complete NG8xxx catalog + tripwire (Phase 12). See `## Current Milestone` below.
...
## Current Milestone: v0.1.0 -- configuration + init generators, nx add support, and the typecheck executor rename
...
### Active (v0.1.0 -- in planning)

Scoped this milestone (full requirements in `.planning/REQUIREMENTS.md`): the executor RENAME `angular-typechecker:angular-typecheck` -> `angular-typechecker:typecheck` (EXEC-01, breaking -> 0.1.0); the `configuration` generator (renamed from `typecheck-configuration`; GEN-01..06); a standalone `init` generator that seeds `nx.json` targetDefaults (GEN-07) and is called by `configuration` (GEN-08); `nx add angular-typechecker` support (GEN-09); and the generator e2e + `nx add` e2e + `-p` set-equality guard (GE2E-01..03, GUARD-01). Phases 12 (catalog + tripwire) and 13 (reference-walking engine) already shipped within this milestone.

The remaining deferred families carried from v0.0.1 (inferred targets INF, the `ng add` Angular CLI install schematic + Angular CLI workspace generator GEN-FUT-01/02, other surfaces SUR, reporters/performance REP incl. the `NgtscProgram` incremental engine + REP-RES-02b, broader support SUP) plus the `totalFilesCount` observability field (OBS-01) remain Out of Scope (below).
...
- `ng add` (Angular CLI) install schematic. (Nx's `nx add` + the `configuration`/`init` generator suite are PROMOTED into v0.1.0 -- see Current Milestone; only the Angular CLI `ng add` path stays deferred.)
...
| Coarse single-target caching for the walk (vs per-leaf multi-target) | Spike 003: N leaves = N compiles whether walked or wired as N targets -- the walk adds NO compute, only coarser warm-cache granularity; per-leaf targets remain a consumer escape hatch; deferred `createNodesV2` inference is the granular auto-target future | Accepted v0.0.4 (spike 005: walk target must use `default` not `production` inputs) |
| v0.1.0: rename executor `angular-typecheck` -> `typecheck` (id `angular-typechecker:typecheck`) | Cleaner, shorter public id; the `typecheck` target name matches the executor; sets up the generator suite | Scoped v0.1.0 (breaking -> minor bump; drives 0.0.3 -> 0.1.0) |
| v0.1.0: generator SUITE -- `configuration` (renamed) + standalone `init`; caching seeded into `nx.json` targetDefaults via `init`, NOT inlined on the target | Idiomatic first-party Nx pattern (`@nx/eslint:lint-project` -> `lintInitGenerator`; `@nx/vitest:configuration` -> `init`), verified against Nx 23 source; keeps per-project `project.json` minimal AND delivers the cacheable value prop with no manual `nx.json` edit (no `nx add`/init existed before) | Scoped v0.1.0 |
| v0.1.0: support `nx add angular-typechecker` (auto-runs `init`); `ng add` (Angular CLI) stays deferred | `nx add` is near-free once `init` exists (Nx invokes the package's registered `init`), giving cacheable-on-install UX; Angular CLI `ng add` is a separate surface (GEN-FUT-02) | Scoped v0.1.0 |
...
---
*Last updated: 2026-07-02 -- Phase 14 COMPLETE: the `configuration` + standalone `init` generators + `nx add angular-typechecker` support shipped (GEN-01..09), config-edit only (`project.json` + `nx.json`; no `generateFiles`). `init` seeds `nx.json` `targetDefaults["angular-typechecker:typecheck"]` with the verbatim WALK-02 block (unscoped id, `default`-not-`production` inputs, whole-entry `??=` don't-clobber); `configuration` invokes `init` then wires ONE minimal `typecheck` target (tsConfig resolution: `--tsConfig` override [existence-probed] -> solution `tsconfig.json` w/ references -> flat leaf by projectType -> located error), collision-by-executor, idempotent merge-preserving; `generators.json` registers both by `factory` (init by literal key, no `ng-add` alias). Verified 5/5 SC, deep code review 0 blockers (2 warnings fixed in `c306eee`), `nx test` 236/31 + `nx build` + `nx lint` (plugin-checks) all green. The 0.1.0 version cut is deferred to the milestone Release PR. Phases 12, 13, 13.1, 14 shipped; only 15 (generator e2e + `-p` guard) remains.*
